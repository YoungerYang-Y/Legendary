package pers.legendary.common.mbg.rbac.service.impl;

import com.baomidou.mybatisplus.extension.service.impl.ServiceImpl;
import org.springframework.stereotype.Service;
import pers.legendary.common.api.business.user.entity.SysUserRoleRelation;
import pers.legendary.common.mbg.rbac.mapper.SysUserRoleRelationMapper;
import pers.legendary.common.mbg.rbac.service.ISysUserRoleRelationService;

/**
 *
 * Description: 针对表【sys_user_role_relation(用户角色关系表)】的数据库操作Service实现
 *
 * @author YangYang
 * @date 2022-03-19 20:07:58
 * @version 1.0.0
 */
@Service
public class SysUserRoleRelationServiceImpl extends ServiceImpl<SysUserRoleRelationMapper, SysUserRoleRelation>
    implements ISysUserRoleRelationService {

}




